...
const at::Tensor& TensorImpl::fw_grad(uint64_t level) const {
  // See TensorImpl::grad() above for explanation about the line below
  if (!autograd_meta_) return impl::GetAutogradMetaFactory()->undefined_tensor();
  return autograd_meta_->fw_grad(level);
}

void TensorImpl::set_fw_grad(at::Tensor& new_grad, const at::Tensor& self, uint64_t level) {
  if (!autograd_meta_) autograd_meta_ = impl::GetAutogradMetaFactory()->make();
  autograd_meta_->set_fw_grad(new_grad, self, level);
}

void TensorImpl::reset_fw_grad(uint64_t level) {
  // !autograd_meta_ => there is not forward grad at this level
  if (!autograd_meta_) return;
  autograd_meta_->reset_fw_grad(level);
}
