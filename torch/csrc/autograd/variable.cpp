...
void AutogradMeta::set_fw_grad(Variable& new_grad, const Variable& self, uint64_t level) {
  if (fw_grad_.contains(level)) {
    ...
    if (new_grad.defined()) {
      ...
      try {
        fw_grad_.value(level).copy_(new_grad);
    ...
    if (keep_new_grad) {
      ...
      if (is_view_) {
        ...
        if (this_view_meta->has_view_fn()) {
          fw_grad_.set_value(this_view_meta->view_fn()(this_view_meta->base_.fw_grad()), level);
        } else {
          const auto& val = this_view_meta->base_.fw_grad().as_strided(self.sizes(), self.strides(), self.storage_offset());
          fw_grad_.set_value(val, level);
        }
      } else {
        // Create a Tensor with the same meta as self
        fw_grad_.set_value(new_with_same_meta(self), level);
      }
      ...
      if (new_grad.defined()) {
        if (is_view_) {
          ...
          try {
            ...
            if (!*fw_grad_.value(level).eq(new_grad).all().data_ptr<bool>()) {
